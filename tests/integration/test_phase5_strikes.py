...
def _world(tmp_path: Path) -> tuple[Path, dict[str, int]]:
    ...
    mid_alice = members_repo.insert(conn, slug="alice", display_name="Alice", status_id=active.id)
    mid_bob = members_repo.insert(conn, slug="bob", display_name="Bob", status_id=active.id)
    conn.close()
...
def test_strike_in_other_semester_isolated(tmp_path: Path) -> None:
    ...
    fa_standing = _run(db_path, "strike", "standing", "alice", "--semester", "FA26")["data"]
